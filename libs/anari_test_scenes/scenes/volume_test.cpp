    ...
    namespace scenes {
        ...
        std::vector<ParameterInfo> VolumeTest::parameters()
        {
            return {
                ...
                {"valueRangeRight", ANARI_FLOAT32, 10.0f, "Sampled values of field are clamped to this range"},
        ...
        void VolumeTest::commit()
        {
            ...
            const float valueRangeLeft = getParam<float>("valueRangeLeft", 0.0f);
            const float valueRangeRight = getParam<float>("valueRangeRight", 10.0f);
            const float densityScale = getParam<float>("densityScale", 1.0f);
            const int volumeDims = 128;
            const size_t numPoints = 10;
            //const float voxelRange[2] = { 0.f, 10.f };
            const float voxelRange[2] = { valueRangeLeft, valueRangeRight };
